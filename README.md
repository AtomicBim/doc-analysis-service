...
### 2. UI-сервис Gradio (ui-service)
- **Технология**: Gradio (Python)
- **Функция**: Простой веб-интерфейс для быстрого доступа
- **Порты**: 7861 (внутри контейнера), 7861 (внешний)
- **Контейнер**: `doc_analysis_ui`

### 3. UI-сервис React (ui-react-service) ⭐ NEW
...
| Сервис | URL | Описание |
|--------|-----|----------|
| 🎨 **React UI** | `http://localhost:7862` | Современный веб-интерфейс (рекомендуется) |
| 🖥️ **Gradio UI** | `http://localhost:7861` | Простой интерфейс для быстрого доступа |
...
**Gradio UI** (`http://localhost:7861`) - для быстрого тестирования:
